...
def run_forecast():
    """
    Function to run the HUXt forecasts with boundary conditions and cone CMEs pulled from the UKMO API.
    ...
    dummymodel = H.HUXt(v_boundary=np.ones(128)*400*(u.km/u.s), simtime=simtime, dt_scale=dt_scale, cr_num=cr,
                        cr_lon_init=cr_lon_init, lon_out=0.0*u.deg, r_min=r_in, r_max=r_out)
    ...
    for cor_input in cor_inputs:
            ...
            if os.path.exists(mapfilepath):
                ...
                if deacc:
                    ...
                    for nlat in range(1, len(vr_lats)):
                        vr_map_deacc[nlat, :], lon_temp = Hin.map_v_inwards(vr_map[nlat, :], 215*u.solRad, vr_longs,
                                                                            r_in)
                ...
                if deacc:
                    # deaccelerate them?
                    v_in, lon_temp = Hin.map_v_inwards(v_in, 215*u.solRad, vr_longs, r_in)
            else:
        ...
        if ambient_success:
            ...
            time = ambient_time
            fig, axs = Hens.plot_ensemble_dashboard_V2(time,
                                                       vr_map,
                                                       vr_longs,
                                                       vr_lats,
                                                       cme_list,
            ...
            fname = os.path.join(savedir, 'plot_' + cor_input + '-HUXt_forecast_' +
            ...
            if det_viz:
                ...
                fname = os.path.join(savedir, 'plot_' + cor_input + '-HUXt_snapshot_' +
                                     forecasttime.strftime("%Y_%m_%dT%H_%M_%S") + '.png')
